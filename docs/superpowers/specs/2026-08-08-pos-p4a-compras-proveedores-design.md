...
| `rtn` | text null | Único si presente (índice parcial) |
| `contacto` | text null | Nombre de la persona de contacto |
| `telefono` | text null | |
| `correo` | text null | |
| `direccion` | text null | |
...
| `proveedor_id` | uuid not null fk → proveedores | `on delete restrict` |
...
- RLS habilitado en las 3 tablas con políticas `for all to authenticated using (true) with check (true)` (patrón admin de P1-P3). Triggers `updated_at` con la función existente `update_updated_at`.
...
## Server Actions (`app/admin/compras/actions.ts` y `app/admin/proveedores/actions.ts`)
...
- CRUD `proveedores` (`crearProveedor`, `actualizarProveedor`, `eliminarProveedor` con guard: no borrar proveedor con compras — FK restrict + mensaje).
...
- **`/admin/proveedores`** — `ProveedoresClient` (CRUD inline/modal, espeja `/admin/clientes`). Link en el sidebar.
...
- **Proveedores:** RTN único si presente; no borrar proveedor con compras (FK restrict + mensaje).
- **Reorden:** respeta variantes (stock por variante); ignora productos sin `stock_minimo`.

## Entrega

- Una migración P4a (tablas `proveedores`/`compras`/`compra_items` + secuencia + `nextval_compra` + `recibir_compra` + `anular_compra` + RLS + triggers `updated_at`), **idempotente** (`if not exists`, `add column if not exists` donde aplique), aplicada por el usuario en el SQL Editor **antes** del push. Smoke SQL corto (`supabase/smoke-pos-p4a.sql`) que verifica estructura, secuencia y funciones, sin crear/borrar datos.
