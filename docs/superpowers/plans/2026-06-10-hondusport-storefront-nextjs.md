...
- [ ] **Step 1: Server Component** `page.tsx` — fetch del producto por id + relacionados (misma cat) + `generateMetadata` con `title = '{nombre} | {site_name}'` y OG image. Si no existe → `notFound()`.
...
- [ ] **Step 3: Commit** `feat(store): página de detalle de producto con SSR y generateMetadata`
