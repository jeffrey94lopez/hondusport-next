...
- [ ] **Step 1:** Reemplazar `<img>` por `next/image` donde falte; añadir `width`/`height`, `loading="lazy"` below-the-fold, `priority` solo al hero. Configurar `images.remotePatterns` en `next.config.ts` para el dominio de Supabase Storage y URLs externas existentes.
- [ ] **Step 2:** A11y: `aria-label` en botones de icono, navegación por teclado en drawers/modales (Escape para cerrar, focus trap), `prefers-reduced-motion` para animaciones.
- [ ] **Step 3:** Portar bloques responsive restantes de `styles.css` **1711-2801** a los módulos correspondientes; verificar que ningún selector global quede huérfano.
- [ ] **Step 4:** `npm run build` sin errores; `npm run lint` limpio; `npx vitest run` verde con 80%+ en `lib/store/**`.
- [ ] **Step 5: Commit** `chore(store): a11y, next/image, responsive y verificación de build`
